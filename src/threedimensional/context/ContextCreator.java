...
public class ContextCreator implements ContextBuilder<Cell> {
	...
	public Context<Cell> build(Context<Cell> context) {
		...
		Grid<Cell> grid = gridFactory.createGrid("grid3d", context, new GridBuilderParameters<Cell>(new StickyBorders(), new SimpleGridAdder<Cell>(), false, params.getInteger("width"), params.getInteger("height"), params.getInteger("depth")));

		float percentageOfImmuneCellsToCreate = 0.10f;
		int size = grid.getDimensions().getWidth() * grid.getDimensions().getHeight() * grid.getDimensions().getDepth();
		...
		Orientation3D bloodVesselOrientation = Orientation3D.valueOf(params.getString("bloodVesselOrientation"));
		...
		case BACK:
			for (int i = 0; i < grid.getDimensions().getWidth(); i++) {
				for (int j = 0; j < grid.getDimensions().getHeight(); j++) {
					BloodCell bloodCell = new BloodCell(grid);
					context.add(bloodCell);
					grid.moveTo(bloodCell, i, j, grid.getDimensions().getDepth()-1);
					context.add(bloodCell);
				}
			}
			break;
		case UP:
			for (int i = 0; i < grid.getDimensions().getWidth(); i++) {
				for (int j = 0; j < grid.getDimensions().getDepth(); j++) {
					BloodCell bloodCell = new BloodCell(grid);
					context.add(bloodCell);
					grid.moveTo(bloodCell, i, grid.getDimensions().getHeight()-1, j);
					context.add(bloodCell);
				}
			}
			break;
		...
		case RIGHT:
			for (int i = 0; i < grid.getDimensions().getHeight(); i++) {
				for (int j = 0; j < grid.getDimensions().getDepth(); j++) {
					BloodCell bloodCell = new BloodCell(grid);
					context.add(bloodCell);
					grid.moveTo(bloodCell, grid.getDimensions().getWidth()-1, i, j);
					context.add(bloodCell);
				}
			}
			break;
		...
		Orientation3D lymphNodeOrientation = Orientation3D.valueOf(params.getString("lymphNodeOrientation"));
		...
		for (int i = 0; i < rccCellsToCreate; i++) {
			...
			do {
				...
				z = random.nextInt(grid.getDimensions().getDepth());
			} while (!grid.moveTo(rcc, x, y, z));
			context.add(rcc);
		}
		...
		for (int i = 0; i < adipocyteCellsToCreate; i++) {
			...
			do {
				...
				y = random.nextInt(grid.getDimensions().getHeight());
				z = random.nextInt(grid.getDimensions().getDepth());
			} while (!grid.moveTo(adipocyte, x, y, z));
			context.add(adipocyte);
		}
		...
		for (int i = 0; i < dendriticCellsToCreate; i++) {
			...
			do {
				...
				y = random.nextInt(grid.getDimensions().getHeight());
				z = random.nextInt(grid.getDimensions().getDepth());
			} while (!grid.moveTo(dendritic, x, y, z));
			context.add(dendritic);
		}
		...
		for (int i = 0; i < plasmacytoidCellsToCreate; i++) {
			...
			do {
				...
				y = random.nextInt(grid.getDimensions().getHeight());
				z = random.nextInt(grid.getDimensions().getDepth());
			} while (!grid.moveTo(plasma, x, y, z));
			context.add(plasma);
		}
